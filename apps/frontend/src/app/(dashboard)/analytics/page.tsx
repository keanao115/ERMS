  ...
  const fetchDashboard = useCallback(
    async (isBackground: boolean) => {
      ...
      try {
        ...
        setLoadError(
          err.response?.data?.message || 'Failed to connect to backend analytics API.'
        );
      } finally {
        setLoading(false);
        setRefreshing(false);
      ...
      <div className="max-w-4xl mx-auto space-y-6 pt-6">
        <div className="glass-panel p-8 rounded-3xl border border-amber-500/30 text-center space-y-4 shadow-2xl">
          ...
          <p className="text-sm text-zinc-300 font-medium">
            Financial & Recipe Costing Analytics Engine is restricted exclusively to Store Managers and Super Administrators.
          </p>
          <p className="text-xs text-zinc-400 max-w-md mx-auto">
            Non-manager accounts do not have permission to view gross margins, recipe COGS, or peak-hour revenue heatmaps.
          </p>
          <div className="pt-2">
  ...
  if (loading) {
    return (
      <div className="h-[calc(100vh-6rem)] flex items-center justify-center text-zinc-400 text-sm gap-2">
        ...
        <span>Loading financial and operational analytics engine...</span>
      </div>
    );
  }
    ...
    return (
      <div className="h-[calc(100vh-6rem)] flex items-center justify-center">
        <div className="glass-panel p-6 rounded-2xl border border-rose-500/30 max-w-md text-center">
          ...
          <p className="text-sm text-white font-semibold mb-1">Failed to load analytics data</p>
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Financial & Recipe Costing Analytics Engine</h1>
          <p className="text-xs text-zinc-400 mt-1">COGS Recipe Costing, Net Profit Margins & Peak-Hour Traffic Heatmap</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="glass-panel p-5 rounded-2xl border border-white/10">
          <span className="text-xs text-zinc-400 font-medium">Gross Revenue</span>
          <p className="text-2xl font-extrabold text-white mt-1">${metrics?.totalRevenue.toFixed(2)}</p>
          <p className="text-[11px] text-zinc-400 mt-1">{metrics?.totalOrders} completed orders</p>
        </div>

        <div className="glass-panel p-5 rounded-2xl border border-white/10">
          <span className="text-xs text-zinc-400 font-medium">Cost of Goods Sold (COGS)</span>
          <p className="text-2xl font-extrabold text-rose-400 mt-1">${metrics?.totalCOGS.toFixed(2)}</p>
          <p className="text-[11px] text-zinc-400 mt-1">Sum of recipe ingredients</p>
        </div>
        ...
        <div className="glass-panel p-5 rounded-2xl border border-white/10">
          ...
          <p className="text-[11px] text-zinc-400 mt-1">Revenue - COGS</p>
        </div>
        ...
        <div className="glass-panel p-5 rounded-2xl border border-white/10">
          ...
          <p className="text-[11px] text-zinc-400 mt-1">Gross Margin Percentage</p>
        </div>
      </div>
      ...
      <div className="glass-panel p-6 rounded-2xl border border-white/10 space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-white">Peak Operational Hours Heatmap Matrix</h2>
          </div>
          ...
          <div className="flex items-center gap-3 text-[11px] text-zinc-400 bg-white/5 border border-white/10 px-3 py-1.5 rounded-xl">
            <span className="font-semibold text-zinc-300">Volume Scale:</span>
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded bg-white/10 border border-white/20"></span> 0
            </span>
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded bg-blue-500/30 border border-blue-500/50"></span> Low (1)
            </span>
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded bg-amber-500/40 border border-amber-500/60"></span> Med (2)
            </span>
            <span className="flex items-center gap-1">
              <span className="w-3 h-3 rounded bg-rose-500/70 border border-rose-400"></span> High (3+)
            </span>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[800px] space-y-1.5 text-xs">
            ...
            >
              <span className="text-left font-bold text-zinc-300">Day</span>
            ...
            {heatmap.map((dayRow, dayIdx) => (
              <div
                ...
                style={{ display: 'grid', gridTemplateColumns: '70px repeat(24, minmax(0, 1fr))' }}
              >
                <span className="text-[11px] font-bold text-zinc-300 text-left">{DAYS_OF_WEEK[dayIdx]}</span>
                {dayRow.map((cell, hIdx) => {
                  ...
                  return (
                    <div
                      key={hIdx}
                      title={`${DAYS_OF_WEEK[dayIdx]} ${hIdx}:00 - ${cell.orderCount} orders ($${cell.revenue.toFixed(2)})`}
      ...
      <div className="glass-panel p-6 rounded-2xl border border-white/10">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-white">Top Best-Selling Dish Velocity</h2>
          </div>
        ...
        {topDishes.length === 0 ? (
          <p className="text-xs text-zinc-500 text-center py-10">No completed orders found.</p>
        ) : (
          <div className="space-y-4">
            {topDishes.map((dish, i) => (
              <div key={dish.menuItemId} className="flex items-center gap-4">
                ...
                <span className="text-xs font-bold text-emerald-400 w-24 text-right">{dish.quantitySold} sold</span>
              </div>
            ))}
          </div>
        )}
